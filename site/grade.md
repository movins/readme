...
![](../public/grade/20190709163354.png)
...
为了提升程序的稳定性和可维护性，经过多次版本的深层次重构，得到了一个稳定的初期版本。
...
使用Flash开发，miniSDK和PC频道内接口调用使用桥接方式兼容，一次开发即可运行于频道内和Web浏览器中；所有模块使用了MVC模式和事件中心机制，界面和数据逻辑完全分离并异步加载。相比以前，加载速度极大提升，动画效果丰富，开发效率提升了一个量级。
...
跨平台框架的开发、所有模块数据接入处理、部分UI开发及发布部署。
...
一次开发同步运行于PC频道和Web浏览器；逻辑与界面、模块与模块异步加载和容错；大视频性能问题（由于开放平台问题至今无法解决）。
